...
useEffect(()=>{
 const fetchUser=async()=>{
    ...
    setFollowing(Array.isArray(response.data.following) ? response.data.following : [])
 }
fetchUser()
},[])
...
{following.length > 0 ? (
  following.map((data, key) => (
    <FollowList key={key} id={data._id} pic={data.pic} username={data.username} />
  ))
) : (
  <p></p>
)}
...
return <div><Link href={`/dashboard/user/${id}`}><div className='w-full hover:bg-[#080808] cursor-pointer flex justify-between'>
